...
INLINE static bool InvalidHandle(Bitu handle)
	{
...
Bitu XMS_AllocateMemory(Bitu size, Bit16u& handle)
	...
	if (size != 0)
		...
		if (mem == 0)
			LOG(LOG_MISC,LOG_ERROR)("XMS:Allocate zero pages with no memory left");
		}
...
Bitu XMS_MoveMemory(PhysPt bpt)
	...
	Bitu length = vPC_rLodsd(bpt+offsetof(XMS_MemMove, length));
	Bitu src_handle = vPC_rLodsw(bpt+offsetof(XMS_MemMove, src_handle));
	union {
		RealPt realpt;
		Bit32u offset;
	} src, dest;
	src.offset = vPC_rLodsd(bpt+offsetof(XMS_MemMove, src.offset));
	Bitu dest_handle = vPC_rLodsw(bpt+offsetof(XMS_MemMove, dest_handle));
	dest.offset = vPC_rLodsd(bpt+offsetof(XMS_MemMove, dest.offset));
	PhysPt srcpt,destpt;
	if (src_handle)
		{
	...
	if (dest_handle)
		...
		destpt = dWord2Ptr(dest.realpt);
	vPC_rMovsb(destpt, srcpt, length);
	return 0;
	}
...
Bitu XMS_ResizeMemory(Bitu handle, Bitu newSize)
	...
	if (MEM_ReAllocatePages(xms_handles[handle].mem, pages))
		{
...
static bool multiplex_xms(void)
	{
	if (reg_ax == 0x4300)			// XMS installed check
		{
		reg_al = 0x80;
		return true;
		}
	if (reg_ax == 0x4310)			// XMS handler seg:offset
		{
...
void SET_RESULT(Bitu res, bool touch_bl_on_succes = true)
	{
...
//	LOG(LOG_MISC,LOG_ERROR)("XMS: CALL %02X",reg_ah);
	...
	case XMS_GET_EMB_HANDLE_INFORMATION:
		...
		break;
	case XMS_RESIZE_ANY_EXTENDED_MEMORY_BLOCK:
		if (reg_ebx > reg_bx)
			LOG_MSG("64MB memory limit!");
		//fall through
	...
	case XMS_DEALLOCATE_UMB:
		...
		reg_bl = UMB_NO_BLOCKS_AVAILABLE;
		break;
	case XMS_QUERY_ANY_FREE_MEMORY:
		reg_bl = XMS_QueryFreeMemory(reg_ax, reg_dx);
		reg_eax &= 0xffff;
		reg_edx &= 0xffff;
...
public:
	XMS(Section* configuration):Module_base(configuration)
		...
		xms_callback = SegOff2dWord(DOS_GetMemory(0x1)-1,0x10);
		...
		DOS_BuildUMBChain(umb_available);
		}
